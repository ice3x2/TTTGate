...
- [ ] #38 unmatched methods. Status: assigned to `fix_supply15` in `../TTTGate-epic61-listeners`, branch `fix/epic61-admin-routes`, base `7a79df4`, after reviewed #36 integration. Own only AdminServer.ts, dedicated tests and ledger. Record actual RED before implementation; use one pathname decision for login CSRF exemption/routing and numeric path consumers while preserving query data, existing numeric acceptance, authentication/Origin/CSRF and static containment. Test unknown POST/PUT/DELETE completion and query-bearing login; include #71 compound/#36 validation regressions.
...
- [ ] #41 v1 ID reuse. Status: root approved the separately bounded early lane after independent research rereview PASS. Assigned to `review_cert_conflict` in `../TTTGate-epic61-control`, branch `fix/epic61-legacy-handler-ids`, base `7a79df4`.
